...
%
%% The mounting angles of wing and tail at design point
%
...
%
%% The pitch stability derivative for the wing-tail combination and the static margin
%
CL_0 = CL_trim;
eta_t = 1;
x_t = l_t2l_w;
...
v_1 = 120 % you can change the value of airspeed to observe the phenomenon
%
 CL_trim_1 = weight/(0.5*rho*(mph2ftpersec*v_1)^2*S_w);
 x_cg = -1.6 :0.2: 1.8 ; % x_cg/mac_w = -0.29 ~ 0.33,
                       % neutral point at x_cg/mac_w = 0.2508

%
Cm_alpha = x_cg/mac_w*CL_alpha_w - ... 
           ((x_t - x_cg)/mac_w)*(S_t/S_w)*eta_t*CL_alpha_t*(1-epsilon_d_alpha);
CL_alpha = CL_alpha_w + (S_t/S_w)*eta_t*CL_alpha_t*(1-epsilon_d_alpha);
SM = -Cm_alpha/CL_alpha*100
%
%% Plot the trimmed angle of attack and elevator deflection 
%   for x_cg/mac_w from -0.3117 ~ 0.3117
%                       

 Cm_0 = (x_cg/mac_w)*CL_alpha_w*(i_w-alpha_CL0_w)/rad2deg + Cm_ac_w;

%
 CL_delta = (S_t/S_w)*eta_t*CL_alpha_t*epsilon_e;
 Cm_delta = (mac_t/mac_w)*(S_t/S_w)*eta_t*Cm_ac_delta- ...
           ((x_t - x_cg)/mac_w)*(S_t/S_w)*eta_t*CL_alpha_t*epsilon_e;
...
%
figure(1)
plot(x_cg/mac_w, alpha_trim*rad2deg, x_cg/mac_w, delta_trim*rad2deg)
title('Trimmed Angles of Attack and Elevator Deflections')
xlabel('x_cg/mac_w')
ylabel('AOA and Elevator deflection Degree')
legend('\alpha','\delta','location','northeast')
grid on
%
%  
