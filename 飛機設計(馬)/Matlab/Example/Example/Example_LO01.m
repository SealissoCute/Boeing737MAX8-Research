...
%
%% (d) Plot the trimmed angle of attack and elevator deflection for airspeed from 80 to 160 mph
%
CL_0 = CL_trim;
Cm_0 = 0;
%
v_1 = 80:10:160;
CL_trim_1 = weight./(0.5*rho*(mph2ftpersec*v_1).^2*S_w);
...
dy_pressure = 0.5*rho*(mph2ftpersec*v_1).^2;
...
grid on
%
%% (e) Plot the trimmed angle of attack and elevator deflection for airspeed
% from 80 to 160 mph when the CG is moved to the location of the a.c. of
% the wing
%
l_w_new = 0;
l_t_new = l_t2l_w +l_w_new;
%
Cm_alpha_new = -(l_w_new/mac_w)*CL_alpha_w - ...
   (l_t_new/mac_w)*(S_t/S_w)*eta_t*CL_alpha_t*(1-epsilon_d_alpha);
Cm_delta_new = (mac_t/mac_w)*(S_t/S_w)*eta_t*Cm_ac_delta- ...
    (l_t_new/mac_w)*(S_t/S_w)*eta_t*CL_alpha_t*epsilon_e;
d0_new = CL_alpha*Cm_delta_new - CL_delta*Cm_alpha_new;
Cm_0_new = Cm_ac_w;
alpha_trim_new = ((CL_trim_1 - CL_0)*Cm_delta_new + CL_delta*Cm_0_new)/d0_new;
delta_trim_new = (-(CL_trim_1 - CL_0)*Cm_alpha_new - CL_alpha*Cm_0_new)/d0_new;
Tail_force = ((S_t/S_w)*eta_t*CL_alpha_t*(1-epsilon_d_alpha)*alpha_trim_new + ...
               CL_delta * delta_trim_new ).* dy_pressure* S_w;
%
figure(2)
yyaxis left
plot(v_1, alpha_trim_new*rad2deg, v_1, delta_trim_new*rad2deg)
title('Trimmed Angles of Attack,Elevator Deflections and Tail Force(CG at a.c. of the wing)')
xlabel('Airspeed (mph)')
ylabel('AOA and Elevator deflection Degree')
...
plot(v_1,Tail_force)
ylabel('Tail force (lb.)')
legend('\alpha','\delta','Tail Force','location','best')
grid on
%  
